...
static int
FractalMap_init(Noddy *self, PyObject *args, PyObject *kwds)
{
  int i;
  Perlin* perlin;
...
static PyMemberDef FractalMap_members[] = {
  {"octaves", T_INT, offsetof(Noddy, octaves), 0, "number of octaves"},
  {"persistence", T_FLOAT, offsetof(Noddy, persistence), 0, "persistence"},
  {NULL}  /* Sentinel */
}
...
static PyObject * value(PyObject *self, PyObject *args) {
  ...
  for octave in range(self.octaves):
    output += (self->persistence ** (self->octaves - octave - 1)) * value(x, size, &self->perlins[octave])
...
static float value(int *x, int size, Perlin *settings) {
  int *fx,*cx, *y;
